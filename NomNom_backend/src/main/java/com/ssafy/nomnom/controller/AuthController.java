...
/**
 * 인증 관련 API를 처리하는 컨트롤러 - 로그인 (이메일/비밀번호 기반)
 */
@RestController
...
public class AuthController {
	...
	/**
	 * [POST] /api/auth/login 이메일과 비밀번호를 받아 로그인하고, 성공 시 JWT 토큰 발급
	 */
	@PostMapping("/login")
	public ResponseEntity<?> login(@RequestBody LoginRequest request) {
		...
		// 5. 보안을 위해 비밀번호 제거 (또는 DTO 사용 권장)
		user.setUserPw(null);

		// 6. token + user 정보를 함께 반환
	...
	public ResponseEntity<?> test(@AuthenticationPrincipal CustomUserDetails userDetails) {
		if (userDetails == null) {
			return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("userDetails is null");
		}
